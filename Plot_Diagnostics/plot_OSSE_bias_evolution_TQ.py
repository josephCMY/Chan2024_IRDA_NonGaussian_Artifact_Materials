...
vnamelist = ['T', 'QVAPOR','QHYDRO' ] #,'QVAPOR','P','PH'] #,'QRAIN','QSNOW','QICE','QGRAUP']
name_list = {}
...
# Adjust subplots
fig.subplots_adjust( top = 0.9, bottom = 0.35, left=0.09, right=0.94,
                      hspace = 0.4 , wspace=0.25 )


# Add on row for innovation statistics
ax_inno = fig.add_subplot([0.09,0.08,0.85,0.15])

# Aggregate data useful for correlation calculations
sBias_dict = {}

# Color ranges
crange = {'T': np.linspace(-0.05,0.05,6), "QVAPOR": np.linspace(-0.02,0.02,6), "QHYDRO": np.linspace(-0.02,0.02,6) }
...
for vv in range(len(vnamelist)):
  ...
  # Special handling for QHYDRO -- normalization wrt QVAPOR instead of QHYDRO
  if vn == 'QHYDRO':
    normalizer = np.sqrt( np.mean(diagnostics['QVAPOR']['NoDA']['xf']['MSE'][:], axis=0) )
  ...
  for i in range(3):
    ...
    axes[i,2].set_ylabel(' ')
    axes[i,2].yaxis.set_major_formatter(myBlankFmt)
  ...
  # Saving sBias outcomes from QVAP and QHYD for correlation calculations
  if vn == 'QHYDRO' or vn == 'QVAPOR':    
    sBias_dict[vn] = model_impact_on_bias / normalizer
...
# # Compute correlations btwn QVAP and QHYD model response sBias
# qvap_perts = sBias_dict['QVAPOR'] - np.mean( sBias_dict['QVAPOR'], axis=0 )
# qhyd_perts = sBias_dict['QHYDRO'] - np.mean( sBias_dict['QHYDRO'], axis=0 )
# print( qhyd_perts.shape )
# qvap_sig = np.std( qvap_perts, axis=0, ddof=1)
# qhyd_sig = np.std( qhyd_perts, axis=0, ddof=1)

# corr = np.mean( qvap_perts * qhyd_perts, axis=0 ) * (nD/(nD+1)) 
# corr /= (qvap_sig * qhyd_sig)
