'''
Script to examine time-aggregated bias contribution terms
'''
...
exptlist = ['FCSTCLOUD','FCSTCLOUD_RAOEI','FCSTCLOUD_AOEI']
...
# Colors
expt_style_dict = {
    'FCSTCLOUD': {'color':'k','style':'-'},
    'FCSTCLOUD_RAOEI': {'color':'dodgerblue','style':'-'},
    'FCSTCLOUD_AOEI':  {'color':'r','style':'-'}
}
...
for vv in range(len(vnamelist)):
    ...
    for expt in exptlist:
        ...
        bias_impact = np.sum(incre_impact_on_bias / normalizer, axis=0)
        axes[0,vv].plot( bias_impact, plvls, color=expt_style_dict[expt]['color'], linestyle=expt_style_dict[expt]['style'], 
                        label = convert_old_exptname_to_manuscript_name(expt))

        # Plot domain-averaged model integration impact on bias
        bias_impact = np.sum(model_impact_on_bias / normalizer, axis=0)
        axes[1,vv].plot( bias_impact, plvls, color=expt_style_dict[expt]['color'], linestyle=expt_style_dict[expt]['style'], 
                        label = convert_old_exptname_to_manuscript_name(expt))
        
        # Plot Pvera;; impact on bias
        bias_impact = np.sum(overall_bias_change / normalizer, axis=0)
        axes[2,vv].plot( bias_impact, plvls, color=expt_style_dict[expt]['color'], linestyle=expt_style_dict[expt]['style'], 
                        label = convert_old_exptname_to_manuscript_name(expt))
    ...
    # Pretty titles
    axes[0,vv].set_title( 'a%d) %s Tot. Incre. $\Delta$sBias' % (vv+1, vn), loc='left')
    axes[1,vv].set_title( 'b%d) %s Tot. Resp. $\Delta$sBias'  % (vv+1, vn), loc='left')
    axes[2,vv].set_title( 'c%d) %s Tot. $\Delta$sBias '          % (vv+1, vn), loc='left')

    axes[0,vv].set_xlabel( '%s Total Incre. $\Delta$sBias' % (vn))
    axes[1,vv].set_xlabel( '%s Total Resp. $\Delta$sBias'  % (vn))
    axes[2,vv].set_xlabel( '%s Total $\Delta$sBias'          % (vn))
    ...
    for i in range(3):
        ...
        axes[i,vv].axvline(0, linestyle=':',color='k')
        # # Handling axis ticks
        # if i < axes.shape[0]-1:
...
fig.subplots_adjust(bottom=0.1, top=0.95, left=0.13, right=0.98, hspace=0.4, wspace=0.1)
...
# Save figure
plt.savefig('figs/bias_components.pdf' )
plt.close()
