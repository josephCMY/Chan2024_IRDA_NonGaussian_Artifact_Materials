...
program convert_meteosat_SEVIRI_L1b

    ! Program to read meteosat-R (16/17) L1b radiances
    ...
    use meteosat_SEVIRI_L1b_mod, only : meteosat_SEVIRI_map_type, meteosat_load_SEVIRI_map, &
                                 make_obs_sequence
    ...
    integer                 :: filecount
    type(meteosat_SEVIRI_map_type) :: map
    type(obs_sequence_type) :: seq
    
    integer :: io, iunit, ifile
    
    ! version controlled file description for error handling, do not edit
    character(len=*), parameter :: source   = 'convert_meteosat_SEVIRI_L1b.f90'
    ...
    character(len=256) :: ir_channel_name
    ...
    namelist /convert_meteosat_SEVIRI_L1b_nml/ l1_files, l1_file_list, &
                                        outputfile, &
                                        lon1, lon2, lat1, lat2, &
                                        x_thin, y_thin, meteosat_num, &
                                        ir_channel_name, &
    ...
    call initialize_utilities('convert_meteosat_SEVIRI_L1b')
    ...
    call find_namelist_in_file('input.nml', 'convert_meteosat_SEVIRI_L1b_nml', iunit)
    read(iunit, nml = convert_meteosat_SEVIRI_L1b_nml, iostat = io)
    call check_namelist_read(iunit, io, 'convert_meteosat_SEVIRI_L1b_nml')
    
    ! Record the namelist values used for the run ...
    if (do_nml_file()) write(nmlfileunit, nml=convert_meteosat_SEVIRI_L1b_nml)
    if (do_nml_term()) write(    *      , nml=convert_meteosat_SEVIRI_L1b_nml)
    ...
    filecount = set_filename_list(l1_files, l1_file_list, "convert_meteosat_SEVIRI_L1b")
    
    ! for each input file
    do ifile=1, filecount
       ! read from netCDF file into a derived type that holds all the information
       call meteosat_load_seviri_map(l1_files(ifile), map, trim(ir_channel_name),meteosat_num)
    
       ! convert derived type information to DART sequence
       call make_obs_sequence(seq, map, lon1, lon2, lat1, lat2, &
                              x_thin, y_thin, meteosat_num, reject_dqf_1, obs_err, &
                              vloc_pres_hPa)
    ...
    end program convert_meteosat_SEVIRI_L1b
