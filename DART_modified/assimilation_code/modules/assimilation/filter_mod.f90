...
!------------------------------------------------------------------------------
use adaptive_obs_error_inflation,    only : apply_aoei, init_aoei, use_aoei
...
AdvanceTime : do
   ...
   write(msgstring, '(A)') 'Before Adaptive Observation Error Inflation'
   call     trace_message(msgstring, 'filter:', -1)

   call init_aoei()
   if (use_aoei) call apply_aoei( state_ens_handle, obs_fwd_op_ens_handle, seq, keys, ens_size , obs_val_index)

   write(msgstring, '(A)') ' After Adaptive Observation Error Inflation'
   call     trace_message(msgstring, 'filter:', -1)
